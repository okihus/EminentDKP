...
function Object:New(parent, name, title, width, height)
	...
	EminentDKP.UI:RegisterForCombatMinimize(f)
	return f
end
...
function Object:CreateContentFrame(parent, name, height)
	local c = CreateFrame("Frame", "EminentDKP_UI_" .. name .. "_Content", parent, BackdropTemplateMixin and "BackdropTemplate") -- frame that contains the actual content
	c:SetFrameLevel(1)
	c:SetBackdrop({
...
function Object:CreateTitleFrame(parent, name, title, width)
	local tf = CreateFrame("Frame", "EminentDKP_UI_" .. name .. "_Title", parent, BackdropTemplateMixin and "BackdropTemplate")
	tf:SetFrameLevel(2)
	tf:SetBackdrop({
		bgFile = AceGUIWidgetLSMlists.background[db.skins[db.currentSkin].background],
		edgeFile = AceGUIWidgetLSMlists.border[db.skins[db.currentSkin].border],
...
EminentDKP.UI:RegisterElement(Object, name)